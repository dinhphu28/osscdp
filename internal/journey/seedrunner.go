...
// SeedRunner drains durable journey_seed_job rows: it claims a job, pages over the
// entry segment's active members enrolling each, persists a cursor per page so a crash
// RESUMES from the last enrolled member, and completes when the population is fully
// backfilled. Restart-safe; concurrent-safe (claim uses FOR UPDATE SKIP LOCKED + a
// time-boxed reclaim, and the cursor writes are claim-fenced). Clone of segment.SeedRunner.
...
// tick; reclaim time-boxes a crashed claim.
...
// WithPageSize overrides the per-page member count (tests exercise multi-page drains).
	...
	for {
		...
		case <-ticker.C:
			if _, err := r.RunOnce(ctx); err != nil && ctx.Err() == nil {
				r.logger.Error("journey seed runner failed", "error", err.Error())
			}
		}
	}
}
...
func (r *SeedRunner) RunOnce(ctx context.Context) (idle bool, err error) {
	now := r.now()
	job, ok, err := r.repo.ClaimJourneySeedJob(ctx, now, r.reclaim)
	if err != nil {
		return false, err
	}
	if !ok {
	...
	for p := 0; p < r.pagesPerClaim; p++ {
		...
		if done {
			if err := r.repo.CompleteSeedJob(ctx, job.TenantID, job.JourneyID, job.ClaimedAt); err != nil {
				return false, err
			}
			if r.OnJobDone != nil {
				r.OnJobDone()
		...
		if err := r.repo.SetSeedJobCursor(ctx, job.TenantID, job.JourneyID, job.Cursor, job.ClaimedAt); err != nil {
			return false, err
		}
	}
	// Bounded pages reached; more remain — unclaim so the next tick continues it.
	return false, r.repo.ReleaseSeedJob(ctx, job.TenantID, job.JourneyID, job.ClaimedAt)
}
